...
// Self Training, from book lesson
class effective_modern_cpp
{
public:
	effective_modern_cpp() {};
	~effective_modern_cpp() = default;
	int lesson1();

};

std::unique_ptr<effective_modern_cpp> pass_through(std::unique_ptr<effective_modern_cpp> p)
{
	return p;
}

int effective_modern_cpp::lesson1()
{
	// memory
	std::auto_ptr<int> ap(new int(1));
	std::unique_ptr<int> ap2(new int(1));
...
void TestEMCPP()
{
	std::unique_ptr<effective_modern_cpp> tc = std::make_unique<effective_modern_cpp>();
	effective_modern_cpp *a = new effective_modern_cpp();
